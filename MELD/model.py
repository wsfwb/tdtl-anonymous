...
class TriModalAdaptiveFusion(nn.Module):
    def __init__(self, hidden_dim: int, dropout: float = 0.0):
        ...
        # Light-weight per-modality adapters so fusion operates on modality-aware features
    ...
    def forward(self, x_a, x_t, x_v):
        ...
        w = F.softmax(w, dim=-1)  # (batch, seq_len, 3)
...
class Transformer_Based_Model(nn.Module):
    def __init__(self, args):
        ...
        self.t_t = TransformerEncoder(d_model=args.hidden_dim, d_ff=args.hidden_dim, n_head=args.n_head, num_layers=args.num_layer, dropout=args.dropout)
        self.a_t = TransformerEncoder(d_model=args.hidden_dim, d_ff=args.hidden_dim, n_head=args.n_head, num_layers=args.num_layer, dropout=args.dropout)
        self.v_t = TransformerEncoder(d_model=args.hidden_dim, d_ff=args.hidden_dim, n_head=args.n_head, num_layers=args.num_layer, dropout=args.dropout)

        self.a_a = TransformerEncoder(d_model=args.hidden_dim, d_ff=args.hidden_dim, n_head=args.n_head, num_layers=args.num_layer, dropout=args.dropout)
        self.t_a = TransformerEncoder(d_model=args.hidden_dim, d_ff=args.hidden_dim, n_head=args.n_head, num_layers=args.num_layer, dropout=args.dropout)
        self.v_a = TransformerEncoder(d_model=args.hidden_dim, d_ff=args.hidden_dim, n_head=args.n_head, num_layers=args.num_layer, dropout=args.dropout)

        self.v_v = TransformerEncoder(d_model=args.hidden_dim, d_ff=args.hidden_dim, n_head=args.n_head, num_layers=args.num_layer, dropout=args.dropout)
        self.t_v = TransformerEncoder(d_model=args.hidden_dim, d_ff=args.hidden_dim, n_head=args.n_head, num_layers=args.num_layer, dropout=args.dropout)
        self.a_v = TransformerEncoder(d_model=args.hidden_dim, d_ff=args.hidden_dim, n_head=args.n_head, num_layers=args.num_layer, dropout=args.dropout)
        ...
        # Adaptive fusion (used for audio-branch fusion)
    ...
    def forward(self, text, audio, video, u_mask, q_mask, dia_len, return_features=False):  # text:(sql, batch, hidden) umask:(batch, sql)
        ...
        # keep initial modality representations for t-SNE visualization
        # note:
        #   audio_init / video_init are "initial modality representations"
        #   before cross-modal interaction, instead of raw untouched inputs.
        audio_init = None
        audio_mid = None
        video_init = None
        
        for _ in range(self.n_rounds):
            # Keep clean residual paths
            text_res = text
            audio_res = audio
            video_res = video
            ...
            # save the first-round initial representations for t-SNE
